 ...
 */
class auth_plugin_redmine extends auth_plugin_base {
    ...
    function __construct() {
        ...
        $this->authtype = 'redmine';
        $this->config = get_config('auth_redmine');
    ...
    function user_login($username, $password) {
        ...
        if ($this->is_internal()) {
            ...
            // Normal case: use external redmine for both usernames and passwords.
            ...
            if ($this->config->passtype === 'plaintext') {
                ...
                return (strtolower($fromdb) == sha1($salt . sha1($extpassword)));
     ...
     */
    function redmine_init() {
        if ($this->is_configured() === false) {
            throw new moodle_exception('auth_redminecantconnect', 'auth_redmine');
        }
    ...
    function get_userinfo($username) {
        ...
        $selectfields = $this->redmine_attributes();

        $result = array();
        // If at least one field is mapped from external redmine, get that mapped data.
        if ($selectfields) {
            $select = array();
            $fieldcount = 0;
    ...
    /**
     * Synchronizes user from external redmine to moodle user table.
     *
     ...
     *
     * Syncing users removes (disables) users that don't exists anymore in external redmine.
    ...
    function sync_users(progress_trace $trace, $do_updates=false) {
        ...
        if (!empty($this->config->removeuser)) {
            ...
            if (!empty($removeusers)) {
                $trace->output(get_string('auth_redmineuserstoremove', 'auth_redmine', count($removeusers)));

                foreach ($removeusers as $user) {
                    if ($this->config->removeuser == AUTH_REMOVEUSER_FULLDELETE) {
                        delete_user($user);
                        $trace->output(get_string('auth_redminedeleteuser', 'auth_redmine', array('name'=>$user->username, 'id'=>$user->id)), 1);
                    } else if ($this->config->removeuser == AUTH_REMOVEUSER_SUSPEND) {
                        $updateuser = new stdClass();
                        $updateuser->id   = $user->id;
                        $updateuser->suspended = 1;
                        user_update_user($updateuser, false);
                        $trace->output(get_string('auth_redminesuspenduser', 'auth_redmine', array('name'=>$user->username, 'id'=>$user->id)), 1);
                    }
                }
            }
            unset($removeusers);
                ...
                if ($update_users) {
                    ...
                    foreach ($update_users as $user) {
                        if ($this->update_user_record($user->username, $updatekeys, false, (bool) $user->suspended)) {
                            $trace->output(get_string('auth_redmineupdatinguser', 'auth_redmine', array('name'=>$user->username, 'id'=>$user->id)), 1);
                        } else {
                            $trace->output(get_string('auth_redmineupdatinguser', 'auth_redmine', array('name'=>$user->username, 'id'=>$user->id))." - ".get_string('skipped'), 1);
                        }
                    }
                    unset($update_users);
                }
        ...
        if (!empty($add_users)) {
            $trace->output(get_string('auth_redmineuserstoadd','auth_redmine',count($add_users)));
            ...
            foreach($add_users as $user) {
                ...
                if ($this->config->removeuser == AUTH_REMOVEUSER_SUSPEND) {
                    if ($olduser = $DB->get_record('user', array('username' => $username, 'deleted' => 0, 'suspended' => 1,
                        ...
                        $trace->output(get_string('auth_redminereviveduser', 'auth_redmine', array('name' => $username,
                            'id' => $olduser->id)), 1);
                        continue;
                    }
                }
                ...
                if ($collision = $DB->get_record_select('user', "username = :username AND mnethostid = :mnethostid AND auth <> :auth", array('username'=>$user->username, 'mnethostid'=>$CFG->mnet_localhost_id, 'auth'=>$this->authtype), 'id,username,auth')) {
                    $trace->output(get_string('auth_redmineinsertuserduplicate', 'auth_redmine', array('username'=>$user->username, 'auth'=>$collision->auth)), 1);
                    continue;
                }
                try {
                    $id = user_create_user($user, false); // It is truly a new user.
                    $trace->output(get_string('auth_redmineinsertuser', 'auth_redmine', array('name'=>$user->username, 'id'=>$id)), 1);
                } catch (moodle_exception $e) {
                    $trace->output(get_string('auth_redmineinsertusererror', 'auth_redmine', $user->username), 1);
                    continue;
                }
    ...
    function user_exists($username) {
        ...
        if (!$rs) {
            print_error('auth_redminecantconnect','auth_redmine');
        } else if (!$rs->EOF) {
     ...
     * compares information saved modified information to external redmine.
     *
    ...
    function user_update($olduser, $newuser) {
        if (isset($olduser->username) and isset($newuser->username) and $olduser->username != $newuser->username) {
            error_log("ERROR:User renaming not allowed in ext redmine");
            return false;
        }
        ...
        if (empty($curruser)) {
            error_log("ERROR:User $olduser->username found in ext redmine");
            return false;
        }
    ...
    public function test_settings() {
        ...
        if (empty($this->config->table)) {
            echo $OUTPUT->notification(get_string('auth_redminenoexttable', 'auth_redmine'), 'notifyproblem');
            return;
        }

        if (empty($this->config->fielduser)) {
            echo $OUTPUT->notification(get_string('auth_redminenouserfield', 'auth_redmine'), 'notifyproblem');
            return;
        }
        ...
        if (!$adodb or !$adodb->IsConnected()) {
            ...
            echo $OUTPUT->notification(get_string('auth_redminecannotconnect', 'auth_redmine'), 'notifyproblem');
            return;
        }
        ...
        if (!$rs) {
            echo $OUTPUT->notification(get_string('auth_redminecannotreadtable', 'auth_redmine'), 'notifyproblem');

        } else if ($rs->EOF) {
            echo $OUTPUT->notification(get_string('auth_redminetableempty', 'auth_redmine'), 'notifyproblem');
            $rs->close();
            ...
            echo $OUTPUT->notification(get_string('auth_redminecolumnlist', 'auth_redmine', implode(', ', $columns)), 'notifysuccess');
            $rs->close();
        }
